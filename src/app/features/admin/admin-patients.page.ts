...
@Component({
  standalone: true,
  imports: [PageHeaderComponent, FormsModule, RouterLink, AdminNavComponent],
  template: `
    <app-page-header title="Patients" subtitle="Browse and inspect patient records." />
    <app-admin-nav />
    <div class="mb-4 grid gap-3 rounded-2xl bg-white p-4 shadow-soft md:grid-cols-4">
      <input class="rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="Search by name/email" [(ngModel)]="search" />
      <button class="btn-secondary md:col-span-1" (click)="applySearch()">Apply</button>
    </div>

    <div class="card-surface overflow-x-auto p-4">
      @if (isLoading && patients.length === 0) {
        <p class="mb-3 text-sm text-slate-500">Loading patients...</p>
      } @else if (showRefreshing) {
        <p class="mb-3 text-xs text-cyan-700">Refreshing patients...</p>
      }
      @if (errorText) {
        ...
        <tbody>
          @for (patient of visiblePatients; track patient.id) {
            <tr class="border-t border-slate-100">
              ...
              <td class="py-2">
                <a class="text-cyan-700 underline" [routerLink]="['/admin/patients', patient.id]">View details</a>
              </td>
            </tr>
          }
        </tbody>
      ...
      <div class="mt-3 flex items-center justify-between text-sm">
        <p class="text-slate-500">Showing {{ visiblePatients.length }} of {{ totalCount }} patients</p>
...
export class AdminPatientsPage {
  ...
  private restoreCachedPatients(): void {
    ...
    try {
      ...
      this.totalCount = typeof parsed.payload.totalCount === 'number' ? parsed.payload.totalCount : this.patients.length;
